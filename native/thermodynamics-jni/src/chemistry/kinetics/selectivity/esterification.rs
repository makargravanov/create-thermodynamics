...
pub fn evaluate_fischer_esterification(
    ...
    let temp_factor = match alcohol.degree {
        SubstitutionDegree::Primary | SubstitutionDegree::Benzylic | SubstitutionDegree::Allylic => {
            if context.is_very_high_temperature() { 0.7 } else { 1.0 }
        }
        SubstitutionDegree::Secondary => {
            if context.is_high_temperature() { 0.5 } 
            else if context.is_very_high_temperature() { 0.2 } 
            else { 0.9 }
        }
    ...
    let mut esterification = ReactivityScore::new(
        ...
        format!(
            ...
            acid.degree, acid_factor,
            alcohol.degree, alcohol_factor,
            temp_factor
        ),
    );
    ...
    let competitors = if matches!(alcohol.degree, 
    ...
    SelectivityResult::from_scores(ReactionType::FischerEsterification, esterification, competitors)
}
...
pub fn esterification_competition(
    ...
    context: &SelectivityContext,
) -> Vec<(usize, f64)> {  // (index, relative_rate)
...
pub fn will_esterify(alcohol: &SiteDescriptor, context: &SelectivityContext) -> bool {
    ...
    if matches!(alcohol.degree, SubstitutionDegree::Primary | SubstitutionDegree::Benzylic) {
        return true;
    }
...
mod tests {
    ...
    fn secondary_temperature_dependence() {
        ...
        assert!(hot.dominant_competitor.is_some() || hot.recommendation == SelectivityRecommendation::Mixed);
    }
