    ...
    F: Fn(&mut MolecularEditor, usize) -> ChemistryResult<()>,
{
    ...
    let descriptor = AromaticRingDescriptor::from_ring_atoms(aromatic.structure, ring_atoms);
    if is_friedel_crafts && descriptor.is_deactivated_for_fc() {
        return Ok(None);
    }
...
pub(crate) fn generate_fc_alkylation(
    ...
    let ring_atoms = &aromatic.site.atoms;
    let descriptor = AromaticRingDescriptor::from_ring_atoms(aromatic.structure, ring_atoms);
    if descriptor.is_deactivated_for_fc() {
        return Ok(None);
    }
...
pub(crate) fn generate_aryl_halide_hydroxide_substitution(
    ...
    let activation = descriptor.compute_snar_activation_delta(site.carbon);
    if activation >= 0.0 {
        return Ok(None);
    }
    let base_ea = 35.0;
