...
class ReactorControllerUiModelTest {
    ...
    fun `snapshot creates flat generated state for selected zone`() {
        ...
        assertEquals(false, state.overviewSelected)
        assertEquals(false, state.zonesSelected)
        assertEquals(true, state.mixtureSelected)
        assertEquals("blocks 27", state.chamberBlocksText)
        assertEquals("ports 2", state.portCountText)
        assertEquals("2 subs", state.selectedZoneMixtureCompactCount)
        assertEquals("2 substances", state.selectedZoneMixtureCount)
